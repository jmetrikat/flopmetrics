...
import subprocess
from multiprocessing import Array, Process, Event, Value
...
class NvidiaProfiler(Profiler):
    ...
    def _nvidiasmi_profiling_process(
        should_run: Value, started: Event, stopped: Event, result_handler: ResultHandler, current_record_step: Array, interval: int  # type: ignore
    ):
        """
    ...
    def __enter__(self):
        assert (
            subprocess.getstatusoutput("nvidia-smi")[0] == 0
    ...
    def to_pandas(self) -> pd.DataFrame:
        ...
        df: pd.DataFrame = pd.DataFrame(
            self.data, columns=["gpu_id", "timestamp", "power", "memory", "record_step"]
        )
